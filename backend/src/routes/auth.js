  ...
  try {
    ...
    if (pages.length === 0) {
      ...
      try {
        ...
        for (const pageId of pageIds) {
          try {
            const pageRes = await axios.get(`https://graph.facebook.com/v25.0/${pageId}`, {
              params: {
                access_token: userToken,
                fields: 'id,name,access_token,picture,instagram_business_account{id,username,name,profile_picture_url},connected_instagram_account{id,username,name,profile_picture_url}',
              },
    ...
    for (const page of pages) {
      ...
      try {
        ...
        console.log('[FB OAuth] 實際呼叫 IG 帳號確認成功:', JSON.stringify(igRes.data));
      } catch (igErr) {
        console.log('[FB OAuth] 實際呼叫 IG 帳號確認失敗:', igErr.response?.data?.error?.message);
      }
    }
